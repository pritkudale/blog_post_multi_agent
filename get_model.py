...
for the CrewAI blog generation system. It has been updated to use a dedicated
function to check the actual model being used by the API endpoint.
"""
...
def get_actual_model_from_api(api_key, base_url):
    """
    (Newly Integrated Tool)
    Makes a live API call to determine the actual model being used by the endpoint.
    
    Args:
        api_key (str): The API key for authentication.
        base_url (str): The API endpoint URL (e.g., a routing service).
    
    Returns:
        dict: A dictionary with the test results, or an error dictionary.
    """
    try:
        # Ensure client can be initialized
        if not api_key or not base_url:
            raise ValueError("OPENAI_API_KEY and OPENAI_API_BASE must be set in the environment.")
            
        client = OpenAI(api_key=api_key, base_url=base_url)
        
        # Send a test request to the endpoint
        response = client.chat.completions.create(
            model='gpt-4o-mini', # This is the requested model
            messages=[{'role': 'user', 'content': 'Hi'}],
            max_tokens=5
        )
        
        # Return the actual model name provided in the API's response
        return {
        ...
        # Return an error dictionary if the test fails
        return {'error': str(e)}
...
def get_model_usage_info():
    ...
    # Test actual model being used by calling the new tool function
    actual_model_info = get_actual_model_from_api(
...
def print_usage_summary():
    ...
    # This section now uses the output from the new tool function
    if 'error' not in info['actual_model_test']:
        ...
        if test['usage']:
            ...
            # This checks if cost information is available, matching your API response
            if 'cost' in test['usage'] and test['usage']['cost'] and 'total_cost' in test['usage']['cost']:
